...
def create_lmdb_for_reds():
    ...
    """
    # train_sharp
    folder_path = 'REDS/train/sharp'
    lmdb_path = 'REDS/train/sharp_with_val.lmdb'
    ...
    # train_sharp_bicubic
    folder_path = 'REDS/train/deLR'
    lmdb_path = 'REDS/train/deLR_with_val.lmdb'
    img_path_list, keys = prepare_keys_reds(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys, multiprocessing_read=True)
    
    # train_mvs
    folder_path = 'REDS/train/mvs'
    lmdb_path = 'REDS/train/mvs_with_val.lmdb'
    img_path_list, keys = prepare_keys_reds_npy(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys, multiprocessing_read=True)
...
def prepare_keys_reds_npy(folder_path):
    """Prepare image path list and keys for REDS dataset.
    ...
    """
    print('Reading image path list ...')
    img_path_list = sorted(list(scandir(folder_path, suffix='npy', recursive=True)))
    keys = [v.split('.npy')[0] for v in img_path_list]  # example: 000/00000000
