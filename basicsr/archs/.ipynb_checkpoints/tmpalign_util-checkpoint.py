...
class TMPAlign(nn.Module):
    ...
    def forward(self, x, lqs, hidden_states=None):
        ...
        aligned_positions = torch.cat([aligned_positions_x, aligned_positions_y], dim=4)  # [8, 15, 64, 64, 2]
        ...
        for fi in range(t):
            ...
            current_aligned_position = aligned_positions[:, fi] # [8, 64, 64, 2]
            current_key = inp_key[:, fi] # [8, 16, 64, 64]
