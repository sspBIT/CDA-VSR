...
class VideoRecurrentMixModel(VideoBaseModel):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            for fi in range(t):
                ...
                if fi % 25 == 0:
                    hidden_states = None
                with torch.no_grad():
                    gt_cuda, hidden_states = net_g_cuda(inp_cuda,mv_cuda,hidden_states, return_hs=True)
                gt_cpu = gt_cuda.cpu()
    ...
    def optimize_parameters(self, scalar, current_iter):
        ...
        scalar.scale(l_total).backward()
        scalar.step(self.optimizer_g)
        scalar.update()
